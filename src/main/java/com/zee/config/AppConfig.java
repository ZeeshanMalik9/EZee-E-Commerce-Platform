...
public class AppConfig {
	...
	// corsconfigurationsource will help to define the cors configuration which is
	// used in the security filter chain
	// to allow the frontend to access the backend by defining the allowed origins,
	// methods, headers etc.
	...
	//// cfg.setAllowedMethods(Collections.singletonList("localhost:3000
	// localhost:4200"));
	...
	private CorsConfigurationSource corsConfigurationSource() {
		return new CorsConfigurationSource() {
			...
			public CorsConfiguration getCorsConfiguration(HttpServletRequest request) {
				...
				// Use patterns for localhost flexibility
				cfg.setAllowedOriginPatterns(Collections.singletonList("http://localhost:*"));

				// Explicitly list HTTP methods instead of "*" when allowCredentials is true
